    ...
    class REoptPostProcessor
      ...
      def run_feature_report(feature_report:, reopt_assumptions_hash: nil, reopt_output_file: nil, timeseries_csv_path: nil, save_name: nil, run_resilience: false)
        ...
        if run_resilience
          ...
          puts "hello7"
          #puts "this is reopt output file #{reopt_output_file}"
          #puts "this is reopt output file directory? #{File.directory? reopt_output_file}"
      ...
      def run_scenario_report(scenario_report:, reopt_assumptions_hash: nil, reopt_output_file: nil, timeseries_csv_path: nil, save_name: nil, run_resilience: false, community_photovoltaic: nil)
        ...
        result = api.reopt_request(reopt_input, @scenario_reopt_default_output_file)['data']
        reopt_output = result['data']
        ...
        if run_resilience
          ...
          # add required inputs for resilience request
          post = {
              "Outage": {
        ...
        # TODO TC - UNCOMMENT AFTER DONE
        #result = adapter.update_scenario_report(scenario_report, reopt_output['data'], @scenario_timeseries_default_output_file, resilience_stats)
        ...
        feature_reports.each_with_index do |feature_report, idx|
          ...
          if !(keep_existing_output && output_exists(@feature_reports_reopt_default_output_files[idx]))
            begin
              ...
              if run_resilience
                run_uuid = reopt_output['outputs']['run_uuid']
                puts "this is run uuid #{run_uuid}"
